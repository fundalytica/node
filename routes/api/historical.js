...
router.get(`${process.env.API_PATH}/v1/historical/:symbol`, async (req, res) => {
    ...
    utils.shellHandler(`${process.env.SCRIPTS_PATH}/historical/historical.py`, ['-s', symbol, '-p', provider], callback)
})
...
router.get(`${process.env.API_PATH}/v1/historical/ath/:symbol`, async (req, res) => {
    ...
    const callback = json => res.json(json)
    utils.shellHandler(`${process.env.SCRIPTS_PATH}/historical/historical.py`, ['-s', symbol, '-p', provider, '--ath'], callback)
})
...
router.get(`${process.env.API_PATH}/v1/historical/dip/:symbol-:dip`, async (req, res) => {
    ...
    const callback = json => res.json(json)
    utils.shellHandler(`${process.env.SCRIPTS_PATH}/historical/historical.py`, ['-s', symbol, '-p', provider, '--dip', dip], callback)
})
